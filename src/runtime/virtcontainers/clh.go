...
func (clh *cloudHypervisor) getAvailableVersion() error {
	...
	if len(versionSplit) < 2 {
		fmt.Printf("versionSplit: %+v", versionSplit)
		return errors.New("Failed to parse cloud-hypervisor version field, fields. Illegal length\n")
	}

	// revision could have aditional commit information separated by '-'
	revisions := strings.SplitN(versionSplit[len(versionSplit) - 1], "-", -1)

	// revisions[0] maybe versionSplit[2] or versionSplit[1]
	versionSplit[len(versionSplit) - 1] = revisions[0]
	revision, err := strconv.ParseUint(revisions[0], 10, 64)
	if err != nil {
		return err
	}
	...
	// A new naming rule will be used when clh version >= v15.0,
	//https://github.com/cloud-hypervisor/cloud-hypervisor/commit/30a01277655bf943a5ccdab67d9d398b0335d669
	if major >= 15 {
		revision = 0
	}

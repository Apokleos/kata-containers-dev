...
func (c *Container) cgroupsCreate() (err error) {
	...
	c.Logger().WithField("cgroups.V1.spec.PatchedOCISpec", spec).Warn("cgroupsCreate()")
	...
	c.Logger().WithField("c.state.CgroupPath", c.state.CgroupPath).Warn("c.state.CgroupPath")
	if cgroups.Mode() == cgroups.Unified {
		resource2 := cgroupsv2.ToResources(spec.Linux.Resources)
		c.Logger().WithField("cgroupsv2.Resources: ", resource2).Warn("cgroupsv2.Resources")
		cg2, err := cgroupsv2.NewManager("/sys/fs/cgroup", c.state.CgroupPath, resource2)
		if err != nil {
			return fmt.Errorf("Could not create cgroupv2 for %v: %v", c.state.CgroupPath, err)
		}
		defer cg2.Delete()
		c.Logger().WithField("cgroups.V2.shim", c.process.Pid).Warn("c.process.Pid")
		if c.process.Pid > 0 {
			if err := cg2.AddProc(uint64(c.process.Pid)); err != nil {
				return err
			}
		}
	} else {
		...
		c.Logger().WithField("c.state.CgroupPath", c.state.CgroupPath).Warn("c.state.CgroupPath")
		c.Logger().WithField("cgroups.V1", cgroups.V1).Warn("cgroups.V1")
		if err != nil {
			return fmt.Errorf("Could not create cgroup for %v: %v", c.state.CgroupPath, err)
		}
		c.Logger().WithField("cgroups.V1.shim", c.process.Pid).Warn("c.process.Pid")
